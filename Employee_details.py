...
class EMPLOEE:
    ...
    def add_emploee_details(self,id,name,salary,project):
        quairy_for_search_id=''' select emp_id from employee_table1  '''
        format_quairy_for_search_id= quairy_for_search_id.format(id)
        value_of_quairy_for_search_id=self.cursor.execute(format_quairy_for_search_id)
        f=False
        for i in value_of_quairy_for_search_id:
            if i.emp_id == id:
                f=True
                break
        if f:
            ...
            quairy = '''  INSERT INTO employee_table1 (emp_id,name,salary,project) values ({0},'{1}',{2},'{3}') '''
            inserquairy = quairy.format(id, name, salary, project)
            if project in self.project:
                self.cursor.execute(inserquairy)
                self.cnxn.commit()
                print("details insert sucsessfully in db")
            else:
                try:
                    raise inserexeption
                except inserexeption as ex:
                    print(ex)
